...
| II. Read-Only by Construction | Two tiers. Discovery: no envelope builder for a mutating command exists, so no argument reaches one. Statements: XMLA's `<Statement>` carries the whole command surface — MDX writeback, DMX, stored-procedure `CALL` — so absence is not achievable and `Execute` validates against a query allowlist instead, including refusing a statement batch. | The gap between the tiers being papered over. It was, initially: the claim read "no argument can reach one" for both, and the test that appeared to support it could not have failed. A gate is weaker than an absence and the docs must not imply otherwise (FR-028c). |
