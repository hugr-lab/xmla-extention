...
- [ ] **T-002** GSSAPI discovery in CMake: `krb5-gssapi` + `krb5` via pkg-config on Linux,
...
- [ ] **T-010** `errors`: the six categories plus `IncompleteMessage` as a subtype of
...
- [ ] **T-011** `redact`: scrubbing anchored to real service-principal classes, not a generic
      ...
      preserve. (FR-030)
- [ ] **T-012** `transport::Channel` seam + `BytesChannel`. Nothing above this line may touch
...
- [ ] **T-020** `dime`: encode/decode a record, four fields each padded separately to 4 bytes.
      Tests: a record whose declared bytes arrived but whose padding has not is
      `IncompleteMessage`, not decoded. (FR-001)
- [ ] **T-021** `dime`: message reassembly across records by `MB`/`CF`/`ME`, returning the
      next offset so a peer packing several messages into one segment does not lose the
      remainder. (FR-005)
- [ ] **T-022** `dime`: content-type negotiation and `check_negotiated`. Test that a server
      selecting binary XML or XPRESS raises `Negotiation`, not `Protocol`. (FR-007..009)
- [ ] **T-023** `transport::MessageStream`: reads driven by declared lengths, buffer held
      across calls, bounded, and the bound's error says which of "never terminated" or
      "desynchronised" applies. (FR-002, FR-006)
- [ ] **T-024** `SocketChannel`: connect with a deadline on every wait; the error names no
      host. (FR-025, FR-030)
- [ ] **T-025** `seal_provider`: the capability probe and two implementations — `gss_wrap_iov`
...
- [ ] **T-026** `sealing`: the 4-byte frame, ciphertext first. BOM as its own frame. Chunking.
...
- [ ] **T-027** `envelopes`: `Authenticate` in the extension namespace, `Discover` and
      ...
      FR-023, FR-028)
- [ ] **T-028** `auth`: the mechanism-agnostic handshake loop, bounded rounds, every response
      fault-checked including the terminal one. (FR-012, FR-013, FR-015)
- [ ] **T-029** `rowset`: parse by local element name; extract SOAP faults. (FR-022)
- [ ] **T-030** `client`: negotiate → authenticate → request; capture and carry `SessionId`;
...
- [ ] **T-031** A test that composes **all three splits at once**: a payload chunked into
...
- [ ] **T-032** Build-time check: no file under `src/xmla/` may include a DuckDB header.
      A build failure, not a review note. (constitution III)
...
- [ ] **T-060** `ci.yml`: lint; C++ unit tests on a linux/macos matrix; a build-matrix
...
- [ ] **T-061** `kerberos.yml`: path-filtered on the auth and sealing sources, buildx layer
...
- [ ] **T-063** `codeql.yml` and ClusterFuzzLite. Fuzz the two byte parsers that consume
      remote input: the DIME record decoder and the sealed-frame reader. A byte parser fed by
      a remote peer is what fuzzing is for.
- [ ] **T-064** `README.md` with the lesser scaup, and the `msolap` distinction in the first
      paragraph.
- [ ] **T-065** `ARCHITECTURE.md`: the layer stack, how messages get split, current status —
